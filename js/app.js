...
import {
  getOeuvres, addOeuvre, updateOeuvre,
...
function openEditProcede(p, mouvementId, texteId, rowEl) {
  ...
  document.getElementById('analyseVariant').value = p.hlVariant || 1;

  // Show existing citation + variant picker
  if (p.citation) {
    const mvtNum = p.mouvement || 1;
    const color = getMvtColor(mvtNum);
    showCitationChoisie(p.citation, p.ref || '', color);
    renderVariantPicker(mvtNum, mouvementId, p.hlVariant || 1);
  } else {
    resetCitationZone();
  }
...
document.getElementById('saveAnalyse').addEventListener('click', async () => {
  ...
  const hlVariant = parseInt(document.getElementById('analyseVariant').value) || 1;

  if (state.editingProcede) {
    // UPDATE existing
    await updateProcede(state.editingProcede.id, { citation, ref, procede, analyse, mouvement: mvtNum, num, wordIndices, hlVariant });
    toast('Procédé modifié !', 'success');
    closeAllModals();
    showTexte(state.currentTexteId);
  } else {
    // ADD new
    const docRef = await addProcede(state.pendingAnalyseMvtId, state.pendingAnalyseTexteId, { num, citation, ref, procede, analyse, mouvement: mvtNum, wordIndices, hlVariant });
    ...
    row.innerHTML = renderProcede({ id: docRef.id, citation, ref, procede, analyse, mouvement: mvtNum, num, hlVariant }, mvtNum, count);
    ...
    if (state.editMode) {
      ...
      rowEl.addEventListener('click', (e) => {
        if (e.target.classList.contains('procede-delete')) return;
        openEditProcede({ id: docRef.id, citation, ref, procede, analyse, mouvement: mvtNum, num, wordIndices, hlVariant }, state.pendingAnalyseMvtId, state.pendingAnalyseTexteId, rowEl);
      });
    }
...
async function getUsedVariants(mouvementId, texteId) {
  const procedes = await getProcedesForMouvement(mouvementId);
  return new Set(procedes.map(p => p.hlVariant).filter(v => v != null));
}

// Affiche le sélecteur de variantes dans la modal
async function renderVariantPicker(mvtNum, mouvementId, selectedVariant) {
  const zone = document.getElementById('variantPickerZone');
  const container = document.getElementById('variantSwatches');
  if (!zone || !container) return;

  zone.style.display = '';
  const variants = MVT_VARIANTS[Math.min(mvtNum, 4)] || MVT_VARIANTS[1];
  const usedVariants = mouvementId ? await getUsedVariants(mouvementId) : new Set();

  // Si on édite, la variante actuelle n'est pas "used"
  if (selectedVariant != null) usedVariants.delete(selectedVariant);

  container.innerHTML = '';
  variants.forEach((color, i) => {
    const variantNum = i + 1;
    const swatch = document.createElement('button');
    swatch.type = 'button';
    swatch.className = 'variant-swatch' +
      (usedVariants.has(variantNum) ? ' used' : '') +
      (variantNum === (selectedVariant || 1) ? ' selected' : '');
    swatch.style.background = color;
    swatch.title = `Variante ${variantNum}`;
    swatch.dataset.variant = variantNum;
    swatch.addEventListener('click', () => {
      container.querySelectorAll('.variant-swatch').forEach(s => s.classList.remove('selected'));
      swatch.classList.add('selected');
      document.getElementById('analyseVariant').value = variantNum;
    });
    container.appendChild(swatch);
  });

  // Sélectionner auto la première variante libre si rien n'est sélectionné
  if (!selectedVariant) {
    const firstFree = [...container.querySelectorAll('.variant-swatch:not(.used)')][0];
    if (firstFree) {
      firstFree.classList.add('selected');
      document.getElementById('analyseVariant').value = firstFree.dataset.variant;
    }
  }
}
...
function resetCitationZone() {
  ...
  document.getElementById('variantPickerZone').style.display = 'none';
  document.getElementById('analyseVariant').value = '1';
...
document.getElementById('citationValider').addEventListener('click', () => {
  ...
  const mvtId = state.pendingAnalyseMvtId || (state.editingProcede?.mouvementId);
  const currentVariant = state.editingProcede ? null : null;
  renderVariantPicker(mvtNum, mvtId, currentVariant);
...
async function refreshHighlights(texteId, mouvements) {
  ...
  for (const p of allProcedes) {
    const mvtNum = p.mvtNum;
    const variant = p.hlVariant || 1;
    const variants = MVT_VARIANTS[Math.min(mvtNum, 4)] || MVT_VARIANTS[1];
    const color = variants[Math.min(variant - 1, variants.length - 1)];
    ...
    if (p.wordIndices && p.wordIndices.length > 0) {
      ...
      for (const segment of segments) {
        ...
        for (let i = 0; i <= words.length - segWords.length; i++) {
          ...
          if (match) {
            for (let j = 0; j < segWords.length; j++) indices.push(words[i + j].index);
            break;
          }
        }
      }
    ...
    for (const idx of indices) {
      if (!highlightMap.has(idx)) highlightMap.set(idx, color);
    }
  }
  ...
  // Reconstruire le HTML mot par mot en regroupant les mots contigus de même couleur
  const tempDiv = document.createElement('div');
  tempDiv.innerHTML = texte.texteIntegral;
  const rawText = tempDiv.innerText || tempDiv.textContent;
  const lines = rawText.split('\n');
  ...
  lines.forEach((line, lineIdx) => {
    ...
    // Collecter les tokens de la ligne avec leur couleur
    const tokens = lineText.split(/(\s+)/);
    const lineTokens = []; // {type:'word'|'space', text, color, wIdx}
    tokens.forEach(token => {
      if (/^\s+$/.test(token)) {
        lineTokens.push({ type: 'space', text: token });
      } else if (token) {
        const color = highlightMap.get(wordIndex) || null;
        lineTokens.push({ type: 'word', text: token, color, wIdx: wordIndex });
        wordIndex++;
      }
    });

    // Regrouper les mots contigus de même couleur en blocs
    // Un "bloc" = séquence de mots (+ espaces internes) de même couleur non-null
    let lineHTML = '';
    let i = 0;
    while (i < lineTokens.length) {
      ...
      // Début d'un bloc coloré — on cherche jusqu'où il s'étend
      // Un bloc se prolonge si le mot suivant (après espaces) a la même couleur
      const blockColor = tok.color;
      let blockContent = tok.text;
      let j = i + 1;
      while (j < lineTokens.length) {
        const next = lineTokens[j];
        if (next.type === 'space') {
          // Regarder si le mot d'après a la même couleur
          const wordAfter = lineTokens[j + 1];
          if (wordAfter && wordAfter.type === 'word' && wordAfter.color === blockColor) {
            blockContent += next.text; // inclure l'espace dans le bloc
            j++;
          } else {
            break;
          }
        } else if (next.type === 'word' && next.color === blockColor) {
          blockContent += next.text;
          j++;
        } else {
          break;
        }
